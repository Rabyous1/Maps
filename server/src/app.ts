...
import { SocketService } from './utils/config/socket';
import { schedulePendingOpportunities } from './utils/jobs/startup-jobs';
...
class App {
    ...
    constructor(controllers: Controller[], port: number) {
        ...
        const io = new SocketIOServer(this.server, {
            cors: {
                origin: process.env.ALLOWED_IPS,
                methods: ['GET', 'POST'],
                credentials: true,
                allowedHeaders: ['cookie'],
            },
        });
        io.engine.use(this.sessionMiddleware);

        this.express.use(this.sessionMiddleware);
        io.engine.use(this.sessionMiddleware as any);
    ...
    private initialiseMiddleware(): void {
        this.express
            .use(express.static(uploadsFolderPath))
            .use('/static', express.static(publicFolderPath))
            .use(express.static(buildFrontendFolderPath))
            .use(bodyParser.json())
            .use(
                cors({
            ...
            .use(
                session({
                    secret: process.env.SESSION_SECRET || 'U3H8Q~qgMXtNHr9BJebt4STz2kzgDN60-ljaTbvc',
